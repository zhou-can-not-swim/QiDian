...
namespace QiDian.Services.SearchLogic
{
    ...
    {
        private const int REQUEST_FULL_PATH = 0x00000004;// 请求返回完整文件路径
        private const int REQUEST_SIZE = 0x00000010; // 请求返回文件大小
        private const int REQUEST_DATE = 0x00000040; // 请求返回修改日期
        private const int SORT_NAME_ASC = 1;// 按文件名升序排序
        ...
        public List<FileEntry> Search(string keyword, int maxResults = 3*27)
        {
            ...
            var r = everythingResults
                ...
                .Select(f =>  new FileEntry() { FullPath = f.FullPath, UsageCount = 0, Score = SearchCommonLogic.CalculateScore(f, kl) })
                .OrderByDescending(x => x.Score)
                .ThenBy(x => x.FileName??x.FileName1)
                .Take(maxResults)
                .ToList();
        ...
        public List<FileEntry> SearchByEveryThing(string keyword, int maxResults)
        {
            ...
            try
            {
                ...
                var sb = new StringBuilder(260);//路径放到sb中

                for (int i = 0; i < count; i++)
                {
                    sb.Clear();
                    Everything_GetResultFullPathNameW(i, sb, sb.Capacity);//获取完整路径 → "D:\照片\2024\生日.jpg"
                    Everything_GetResultSize(i, out long size);           //大小
                    Everything_GetResultDateModified(i, out long fileTime);//修改文件的时间
